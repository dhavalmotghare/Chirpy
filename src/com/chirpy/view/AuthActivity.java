...
public class AuthActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		commonHttpOAuthConsumer = new CommonsHttpOAuthConsumer(TwitterManager.CONSUMER_KEY,TwitterManager.CONSUMER_SECRET);
		commonHttpOAuthConsumer.setMessageSigner(new HmacSha1MessageSigner());
		authProvider = new CommonsHttpOAuthProvider(TwitterManager.TWITTER_TOKEN_REQUEST_URL,
    ...
    private class RetrieveRequestToken extends AsyncTask<Context, String, String> {
            ...
            protected String doInBackground(Context... params) {
                ...
                try {
                	return authProvider.retrieveRequestToken(commonHttpOAuthConsumer, TwitterManager.CALLBACK_URL);
                } catch (Exception e) {
            ...
            protected void onPostExecute(String result) {
                ...
                if (!TextUtils.isEmpty(result)) {
                    /** saving the token */
                    RequestToken requestToken = new RequestToken(commonHttpOAuthConsumer.getToken(), commonHttpOAuthConsumer.getTokenSecret());
                    TwitterManager.getInstance().saveRequestToken(requestToken);
                    this.context.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(result)));// launching the browser
                }
            }
	...
	private void retreiveAndStoreToken(Uri uri){
		...
		if (uri != null && uri.toString().startsWith(TwitterManager.CALLBACK_URL)) {
			...
			try {
				...
				commonHttpOAuthConsumer.setTokenWithSecret(requestToken.getToken(), requestToken.getTokenSecret());
    ...
    private class RetrieveAccessToken extends AsyncTask<Context, String, Boolean> {
            ...
            protected void onPostExecute(Boolean result) {
                    if (result) {
                    	...
                    	Toast.makeText(getBaseContext(), "Failed to authorize !!",Toast.LENGTH_LONG).show();
                    }
            }
