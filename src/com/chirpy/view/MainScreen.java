...
public class MainScreen extends Activity implements OnItemClickListener, ActionBar.TabListener {
    ...
    private static final String[] PROJECTION = new String[] {
            TweetDatabase.Tweets._ID, TweetDatabase.AUTHOR_ID,TweetDatabase.MESSAGE,TweetDatabase.IMG_URL, 
	...
	private void updateLists(){
		...
		mCursor = getContentResolver().query(contentUri, PROJECTION,selection,
				new String[] {
		   ...
		   public boolean setViewValue(View view, Cursor cursor, int columnIndex){
			   if(cursor.getCount() > 0){
			       if(view.getId() == R.id.tweet_img){
			           ...
			           drawableManager.fetchDrawableOnThread(url,((ImageView)view));
	...
	private void setupTabs() {
		/** order of tabs
		 * MODE_TIMELINE = 0; MODE_MENTIONS = 1;MODE_MY_TWEETS = 2;
		 */
		if (actionBar != null && actionBar.getTabCount() == 0){
			actionBar.addTab(actionBar.newTab().setText(this.getResources().getString(R.string.timeline)).setTabListener(this));
			actionBar.addTab(actionBar.newTab().setText(this.getResources().getString(R.string.mentions)).setTabListener(this));
			actionBar.addTab(actionBar.newTab().setText(this.getResources().getString(R.string.mytweets)).setTabListener(this));
		}
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_refresh:
			if(menuRefresh == null) menuRefresh = item;
			refreshData();
			return true;
	...
	/**
	 * Either we are launching the application or the user has asked us to refresh the data
	 * 
	 */
     ...
     * This alarm will be received by {@link AlarmReceiver}
     * @return the Intent
     */
